...
class UniversalPointInTimeTest extends TestCaseWithProphecy
{
     ...
     * @test
     */
    public function it_raise_exception_when_comparing_two_different_point_in_time(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Can not operate on two different point in time classes');

        $pointInTime = UniversalPointInTime::now();
        $anotherPointInTime = $this->prophesize(PointInTime::class)->reveal();

        $pointInTime->equals($anotherPointInTime);
    }

    /**
     * @test
     */
    public function it_raise_exception_when_checking_after_two_different_point_in_time(): void
    {
        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Can not operate on two different point in time classes');
        ...
        $pointInTime->after($anotherPointInTime);
    }
